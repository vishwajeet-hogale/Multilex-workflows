...
#for l in all_divs:
    ...
    def dealstreetasia():
        try:
            scrapper_name = 'dealstreetasia'
            print(scrapper_name)
            Errors[scrapper_name]=[]
                
            url = f'https://www.dealstreetasia.com/section/ipos'
            ...
            links = []
            try:
                link =soup.find("div", {"class": 'section-hero card'}).find("a")["href"]
                if link[0] == '/':
                    link = domain_url + link[1:]
                # Filtering advertaisment links
                link_start = domain_url
                ...
                for div in soup.find_all('div',{"class" : 'card'}):
                    ...
                    if link.startswith(link_start):
                        links.append(link)
                for div in soup.find_all('div',{'class': 'main-list-image col-auto col'}):
                    link = div.find('a')['href']
                    # Checking the link if it is a relative link
                    if link[0] == '/':
                        link = domain_url + link[1:]
            ...
            def getarticle(link):
                ...
                # Scraping the published date
                try:
                    date_ele = l_soup.find("p", {"class":'Story_date__RJUPd'})
                    date_text = date_ele.text
                    date_text = (datetime.strptime(date_text,"%d %B, %Y")).strftime("%d-%m-%Y")       
                    data.append(date_text)
                except:
                    err["link"]=link
                    err['published_date']="Error"
                ...
                # Scraping the paragraph
                try:
                    para_ele = l_soup.find('div', {'class':'Story_text_content__SUGxB'})
                    para_ele =para_ele.find_all('p')
                    para_text =""
                    for index, p in zip(range(0,10),para_ele):
                        para_text = para_text + p.text.strip("\n ")
