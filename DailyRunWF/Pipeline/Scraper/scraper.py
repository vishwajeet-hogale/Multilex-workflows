...
def multilex_scraper(input_dir, output_dir):
    ...
    def MoneyControl():
        try:
            ...
            try:
                ...
                err = "moneycontrol: err: Failed to access search url: " + \
                    queryUrl + " and convert it to soup object"
                err_logs.append(err)
                exit()
            ...
            for link in links:
                ...
                try:
                    ...
                    parsedSource = BeautifulSoup(
                        pageSource.content, "html.parser", from_encoding="iso-8859-1")
                except:
                    ...
                    err_logs.append(err)
                    continue
                requiredDiv = parsedSource.find(
                    "div", class_="content_wrapper arti-flow")
                if not requiredDiv:
        ...
        try:
            ...
            try:
                ...
                for i in soup.find_all("item"):
                    ...
                    l1 = re.findall(
                        r'''(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])''', i.text)[0]
                    l = l1[0]+"://"+l1[1]+l1[2]
                    links.append(l)
            except:
    ...
    def korea():
        try:
            ...
            for link in links:
                ...
                month = re.findall(
                    r'''(Jan(uary)?|Feb(ruary)?|Mar(ch)?|Apr(il)?|May|Jun(e)?|Jul(y)?|Aug(ust)?|Sep(tember)?|Oct(ober)?|Nov(ember)?|Dec(ember)?)''', pdate)
                ...
                # Text of article
                text.append(
                    soup.find("div", {"class": "view_con article"}).text)
                # print(text)
                # Scrapped date
        ...
        try:
            ...
            try:
                ...
                for l in links:
                    ...
                    t = sp
                    x = sp.find("h1", {
                                "class": "text__text__1FZLe text__dark-grey__3Ml43 text__medium__1kbOh text__heading_2__1K_hh heading__base__2T28j heading__heading_2__3Fcw5"})
                    if x is not None:
                        n = x.text
                    else:
                        n = None
    ...
    def TradingChart():
        try:
            ...
            for link in links:
                ...
                # Published Date
                div = soup.find(
                    "div", {"class": "news_story m-cellblock m-padding"})
                t = ""
        ...
        try:
            ...
            for keyword in tkeywords:
                ...
                try:
                    ...
                    err = "einnews: err: Failed to access search url: " + \
                        queryUrl + " and convert it to soup object"
                    err_logs.append(err)
                    continue
        ...
        try:
            ...
            try:
                ...
                for item in soup.find_all("item"):
                    ...
                    description.append(html.unescape(
                ...
                for i in range(0, len(description)-1):
                    ...
                    art = art.replace('â€¦', '').replace('<span class="match">', '').replace(
                        '</span>', '').replace('\n', '').replace('     ', '')
                    art = art.replace(",,", ",")
    ...
    def xinhuanet():
        try:
            try:
                ...
                logging.info(
                    "www.xinhuanet.com: invoking requests.url()=" + url)
                page = requests.get(url)
                logging.info(
                    "www.xinhuanet.com: completed invoking requests.url()=" + url)
    ...
    def kontan(keyword):
        try:
            ...
            try:
                ...
                url1 = "https:"
                logging.info(
                    "www.kontan.co.id: invoking requests.url()=" + url)
                page = requests.get(url)
                logging.info(
                    "www.kontan.co.id: completed invoking requests.url()=" + url)
                soup = BeautifulSoup(page.content, 'lxml')
    ...
    def German():
        try:
            ...
            for link in links:
                ...
                title.append(
                    soup.find("span", {"class": "seitenkopf__headline--text"}).text)
                # print(title)
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "metatextline"}).text)
                # print(pub_date)
    ...
    def Japannews():
        ...
        #                 print(m[2].text)
            ...
            japanese = pd.DataFrame(list(zip(title, links, dates, s_date, text)),
                                    columns=['title', 'link', 'publish_date', 'scraped_date', 'text'])
            japanese['publish_date'] = pd.to_datetime(
                japanese['publish_date'], errors='coerce', utc=True).dt.strftime('%d-%m-%Y')
    ...
    def Romania():
        try:
            try:
                ...
                logging.info(
                    "Romania: Completed invoking requests.url()=" + url)
    ...
    def Swedish():
        try:
            ...
            try:
                ...
                page = requests.get(url).content
                logging.info(
                    "Swedish: Completed invoking requests.url()=" + url)
                unicode_str = page.decode("utf-8")
                encoded_str = unicode_str.encode("ascii", 'ignore')
                soup = BeautifulSoup(encoded_str, "html.parser")
                ...
                for link in links:
                    ...
                    y = sp.findAll(
                        "div", {"class": "Meta-part Meta-part--published"})
                    for x in y:
                        x = x.get_text()
                        x = re.sub('[^0-9-:.]+', ' ', x)
                        dates.append(x)
    ...
    #         get_ipython().system('pip install googletrans==3.1.0a0')
            ...
            fin = pd.DataFrame(list(zip(title, link, publish_date, scraped_date)),
                               columns=['title', 'link', 'publish_date', 'scraped_date'])
            fin['publish_date'] = pd.to_datetime(
                fin['publish_date'], errors='coerce', utc=True).dt.strftime('%d-%b-%Y' " " "%H:%M:%S")
        ...
        try:
            try:
                ...
                for l in links:
                    ...
                    title.append(ementa)
                    y = sp.find("div", {
                                "class": "field field--name-field-date field--type-datetime field--label-hidden field__item"})
                    if y is not None:
                        d = str(y.text).strip()
                        d = datetime.strptime(
                            d, "%d %B %Y").strftime("%Y-%m-%d")
                    else:
                        d = None
                    dates.append(d)
                    z = sp.find("div", {
                ...
                df2 = pd.DataFrame(
                    {"text": text, "link": links, "publish_date": dates, "scraped_date": s_date, "title": title})
    ...
    def romania_insider():
        try:
            ...
            try:
                ...
                err = "romania_insider: err: Failed to access search url: " + \
                    queryUrl + " and convert it to soup object"
                err_logs.append(err)
            ...
            for link in links:
                ...
                if parsedSource.find("div", class_="field field--name-field-date field--type-datetime field--label-hidden field__item"):
                    sourceDateTimeTag = parsedSource.find(
                        "div", class_="field field--name-field-date field--type-datetime field--label-hidden field__item")
                else:
                    ...
                    err_index.append(link)
                    continue
                sourceDateTime = datetime.strptime(
                    sourceDateTimeTag.text, "%d %B %Y").strftime("%Y-%m-%d")
            ...
            romania_insiderDF = romania_insiderDF.drop_duplicates(subset=[
                                                                  "link"])
    ...
    def cnbc1():
        try:
            ...
            try:
                ...
                err = "cnbc: err: Failed to access search url: " + \
                    queryUrl + " and convert it to soup object"
                err_logs.append(err)
                exit()
            ...
            for eachItem in parsedSource.find_all("item"):
                ...
                articleText = str(eachItem.find("description").text).split(
                    "CDATA[ ", maxsplit=2)[0].replace("#039;", "")
                ArticleBody.append(articleText)
    ...
    def Carteira():
        try:
            ...
            try:
                ...
                df = pd.DataFrame({"Article": body, "Link": link, "Publish Date": date,
                                  "Scrape Date": scraped_date, "Title": title})
            ...
            dic = {"text": body, "link": link, "publish_date": date,
                   "scraped_date": scraped_date, "title": title}
    ...
    def Kontan1():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "fs14 ff-opensans font-gray"}))
                # print(pub_date)
                ...
                final_links.append(link)
            df = pd.DataFrame({"text": text, "link": final_links,
                              "publish_date": scraped_date, "scraped_date": scraped_date, "title": title})
            if df.empty:
    ...
    def franchdailynews():
        try:
            ...
            df2 = pd.DataFrame({"text": body, "link": link, "publish_date": date,
                               "scraped_date": scraped_date, "title": title})
            df = FilterFunction(df2)
    ...
    def Koreatimes():
        try:
            ...
            all_divs = soup.find_all(
                "div", {"class": "list_article_headline HD"})
            for div in all_divs:
                links.append(div.a["href"])
            # Fetch all the necessary data
            # print(links)
            ...
            for link in links:
                ...
                if (soup.find("div", {"class": "view_headline HD"}) == None):
                    continue
                title.append(
                    soup.find("div", {"class": "view_headline HD"}).text)
                # print(title)
                # Published Date
                pdate = soup.find_all("div", {"class": "view_date"})[0].text
                pdate = "-".join(pdate.split(":")
                                 [1].strip().split(" ")[0].split()[0].split("-")[::-1])
  ...
  # CombineFunction()
    ...
    def zdnet():
        try:
            ...
            for link in links:
                ...
                title.append(
                    soup.find("header", {"class": "storyHeader precap-variation article"}).h1.text)
                # print(title)
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "byline-details"}).time.text)
                # print(pub_date)
    ...
    def arabNews():
        try:
            ...
            keywords = ['IPO', 'pre-IPO', 'Public',
                        'Initial', 'Offering', 'initial']
            ...
            for keyword in tkeywords:
                ...
                try:
                    ...
                    err = "arabNews: err: Failed to access search url: " + \
                        queryUrl + " and convert it to soup object"
                    err_logs.append(err)
                    continue
                # with open("response.html", "wb") as f:
                ...
                for link in links:
                    ...
                    if parsedSource.find("div", class_="entry-date"):
                        sourceDateTimeTag = parsedSource.find(
                            "div", class_="entry-date").find("time")
                    else:
                        ...
                        continue
                    sourceDateTime = datetime.strptime(
                        sourceDateTimeTag.text, "%B %d, %Y %H:%M").strftime("%Y-%m-%d")
                    # print(sourceDateTime)
                    ArticleDates.append(sourceDateTime)
                    ScrapeDates.append(datetime.today().strftime("%d-%m-%Y"))
                    # print(ArticleDates)
                    if parsedSource.find("div", class_="entry-content"):
                        textBodyDiv = parsedSource.find(
            ...
            try:
                ...
                for keyword in tkeywords:
                    ...
                    try:
                        ...
                        err = "chosun: err: Failed to access search url: " + \
                            queryUrl + " and convert it to soup object"
                        err_logs.append(err)
                        continue
                    # with open("response.html", "wb") as f:
                ...
                # print(links)
                    ...
                    for link in links:
                        ...
                        sourceDateTime = datetime.strptime(
                            sourceDateTime, "%B %d, %Y %H:%M").strftime("%Y-%m-%d")
                        ArticleDates.append(sourceDateTime)
                        ScrapeDates.append(
                            datetime.today().strftime("%d-%m-%Y"))
                    # print(ArticleDates)
                        try:
                            ...
                            continue
                        for item in textBodyDiv.find_all("p"):
                            articleText = textBodyDiv.text.replace(
                                item.text, "")
                        # print(articleText)
                        ArticleBody.append(articleText)
                    scrapedData["publish_date"] = ArticleDates
        ...
        try:
            ...
            for link in links:
                ...
                if (soup.find("div", {"class": "metrics-channel light-text with-border"}) == None):
                    continue
                pub_date.append(soup.find(
                    "div", {"class": "metrics-channel light-text with-border"}).time.text)

                # Title of article
                title.append(soup.find("h1", {
                             "class": "fs-headline speakable-headline font-base font-size should-redesign"}))
                # print(title)
            ...
            try:
                ...
                for keyword in tkeywords:
                    ...
                    try:
                        ...
                        parsedSource = BeautifulSoup(
                            pageSource.content, "html.parser")
                    except:
                        err = "kng: err: Failed to access search url: " + \
                            queryUrl + " and convert it to soup object"
                        err_logs.append(err)
                        continue
            ...
            try:
                ...
                for keyword in tkeywords:
                    ...
                    try:
                        ...
                        err = "pymnts: err: Failed to access search url: " + \
                            queryUrl + " and convert it to soup object"
                        err_logs.append(err)
                        continue
                    ...
                    for link in links:
                        ...
                        try:
                            ...
                            continue
                        try:
                            sourceDateTime = parsedSource.find(
                            ...
                            continue
                        sourceDateTime = datetime.strptime(
                            sourceDateTime["datetime"], "%Y-%m-%d").strftime("%d-%m-%Y")
                        ArticleDates.append(sourceDateTime)
                        ScrapeDates.append(
                            datetime.today().strftime("%d-%m-%Y"))
                        try:
                            textBodyDiv = parsedSource.find(
                                "div", id=re.compile("pymnts-content"))
                        except:
        ...
        try:
            try:
                ...
                try:
                    ...
                    err = "toi: err: Failed to access main url: " + \
                        url + " and convert it to soup object"
                    err_logs.append(err)
                    return None
                ...
                for link in links:
                    try:
                        ...
                        err = "toi: err: Failed to retrieve data from link: " + \
                            link + " and convert it to soup object"
                        err_logs.append(err)
                        continue
            ...
            try:
                ...
                for keyword in tkeywords:
                    ...
                    try:
                        ...
                        err = "wealthx: err: Failed to access search url: " + \
                            queryUrl + " and convert it to soup object"
                        err_logs.append(err)
                        continue
                    # with open("response.html", "wb") as f:
                ...
                # print(links)
                    ...
                    for link in links:
                        ...
                        try:
                            sourceDateTimeTag = parsedSource.find(
                                "span", class_="meta-date date updated")
                        except:
                            ...
                            continue
                        sourceDateTime = datetime.strptime(
                            sourceDateTimeTag.text, "%B %d, %Y").strftime("%Y-%m-%d")
                        ArticleDates.append(sourceDateTime)
                        ScrapeDates.append(
                            datetime.today().strftime("%d-%m-%Y"))
                    # print(ArticleDates)
    ...
    def AFR():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "_2cdD4"}).time.text)
                # print(pub_date)
    ...
    def indonesia():
        ...
        try:
            ...
            for link in links:
                ...
                #   continue
                pub_date.append(
                    soup.find("i", {"class": "fa fa-clock-o"}).text)
        ...
        try:
            try:
                ...
                try:
                    ...
                    err = "asiainsurancereview: err: Failed to access main url: " + \
                        url + " and convert it to soup object"
                    err_logs.append(err)
                    return None
                ...
                for link in links:
                    try:
                        ...
                        err = "asiainsurancereview: err: Failed to retrieve data from link: " + \
                            link + " and convert it to soup object"
                        err_logs.append(err)
                        continue
    ...
    def economic_times():
        try:
            ...
            for link in links:
                ...
                title.append(
                    soup.find("h1", {"class": "artTitle font_faus"}).text)
                # print(title)
                # Published Date
                ...
                # text.append(t)
                text.append(
                    soup.find("div", {"class": "pageContent flt"}).text)
                # print(text)
            ...
            try:
                ...
                for link in links:
                    try:
                        ...
                        err = "prnewswire: err: Failed to retrieve data from link: " + \
                            link + " and convert it to soup object"
                        err_logs.append(err)
                        continue
        ...
        try:
            try:
                ...
                for link in links:
                    ...
                    title.append(
                        soup.find("h1", {"class": "page-header"}).text)
                    ...
                    text.append(soup.find("div", {
                                "class": "field field--name-body field--type-text-with-summary field--label-hidden field--item"}).text)
    ...
    #             print(page.content)
            ...
            for link in links:
                ...
                title.append(
                    soup.find("div", {"class": "headline story-pg"}).h1.text)
                pub_date.append(soup.find("p", {"class": "date"}).text)
                text.append(
                    soup.find("div", {"class": "story bot-15 relative"}).text)
        ...
        try:
            ...
            for link in links:
                try:
                    ...
                    err = "err: Failed to retrieve data from link: " + \
                        link + " and convert it to soup object"
                    err_logs.append(err)
                    continue
    ...
    def vccircle():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "publish-time"})["content"])
                # print(pub_date)

                # Title of article
                title.append(
                    soup.find("div", {"class": "premium-txt-container"}).text)
                # print(title)
    ...
    def zawya():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "article-date"}).text)
                # print(pub_date)
            ...
            try:
                ...
                for keyword in tkeywords:
                    ...
                    try:
                        ...
                        err = "phnompenhpost: err: Failed to access search url: " + \
                            queryUrl + " and convert it to soup object"
                        err_logs.append(err)
                        continue
                    # with open("response.html", "w") as f:
                    ...
                    for item in parsedSource.find_all("li", class_="search-result"):
                        ...
                        if currentArticleLink[0] == "/":
                            ...
                            links.append(currentArticleLink)
                        titles.append(currentArticleTitle)
                        currentArticleDateText = str(item.find(
                            "div", class_="posted-date").find("span").text).split("by")[0].strip()
                        try:
                            currentArticleDate = datetime.strptime(currentArticleDateText,
            ...
            try:
                ...
                for keyword in tkeywords:
                    ...
                    try:
                        ...
                        err = "scmp: err: Failed to access search url: " + \
                            queryUrl + " and convert it to soup object"
                        err_logs.append(err)
                        continue
                    # with open("response.html", "wb") as f:
                    ...
                    for item in parsedSource.find_all("li", class_="search-results__item item"):
                        requiredTag = item.find(
                            "span", class_="content-link__title")
                        currentArticleTitle = requiredTag.text
                        # print(currentArticleTitle)
                        currentArticleLink = item.find(
                            "a", class_="content__content-link content-link")["href"]
                        # print(currentArticleLink)
                        if currentArticleLink[0] == "/":
                            links.append(domainUrl + currentArticleLink)
                        else:
    ...
    def livemint():
        try:
            ...
            for link in links:
                ...
                try:
                    para_ele = (l_soup.findAll(
                        "ul", {"class": para_ul_class}))[-1]
                    data.append(para_ele.text)  # Need to make this better
                except:
                    new_para_p_class = "summary"
                    try:
                        para_ele = (l_soup.findAll(
                            "p", {"class": new_para_p_class}))[-1]
                        data.append(para_ele.text)  # Need to make this better
                    except:
                        err = scrapper_name + ": err: Failed to find paragraph in page. Link: " + link
                        err_logs.append(err)
    ...
    def guardian():
        try:
            ...
            try:
                ...
                err = "err: Failed to access main url: " + \
                    url + " and convert it to soup object"
                err_logs.append(err)
                return None
    ...
    def azernews():
        try:
            ...
            for link in links:
                ...
                try:
                    title_ele = l_soup.find(
                        "div", {"class": "article-content-wrapper"}).h2
                    data.append(title_ele.text)
                except:
                    err = scrapper_name + ": err: Failed to find title in page. Link: " + link
                    err_logs.append(err)
        ...
        try:
            ...
            for link in links:
                try:
                    ...
                    # Published Date
                    pub_date.append(
                        soup.find("div", {"class": "data-publicacao"}).text)

                    # Title of article
                    title.append(
                        soup.find("div", {"class": "cell titulo"}).h1.text)

                    # Text of article
                    text.append(
                        soup.find("div", {"class": "cell chamada"}).h2.text)
    ...
    def elicudadona():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "time-ago time-now"})["data-date"])
                # print(pub_date)
    ...
    def lastampa():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("time", {"class": "story__date"})["datetime"])
    ...
    def liputan6():
        try:
            ...
            all_divs = soup.find_all(
                "a", {"class": "ui--a articles--iridescent-list--text-item__title-link"})
            for div in all_divs:
                links.append(div["href"])
            # Fetch all the necessary data
            # print(links)
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(soup.find(
                    "time", {"class": "read-page--header--author__datetime updated"})["datetime"])

                # Title of article
                title.append(
                    soup.find("h1", {"class": "read-page--header--title entry-title"}).text)

                # Text of article
                text.append(
                    soup.find("div", {"class": "article-content-body__item-content"}).p.text)
    ...
    def milenio():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "content-date"}).time.text)
                ...
                # Text of article
                text.append(
                    soup.find("div", {"class": "media-container news"}).p.text)
    ...
    def supchina():
        try:
            ...
            for link in links:
                ...
                try:
                    date_text = "".join(
                        ((" ".join((date_ele.text.split(" "))[1:])).split("\t"))[0])
                    #date_text = (date_text.split('/'))[-1]
                    #date_text = date_text.replace(" Updated: ", "")
                    # The date_text could be further modified to represent a proper date format
                    data.append(date_text)
    ...
    def aljazeera():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "date-simple css-1yjq2zp"}).text)

                # Title of article
                title.append(
                    soup.find("header", {"class": "article-header"}).h1.text)

                # Text of article
                div = soup.find(
                    "div", {"class": "wysiwyg wysiwyg--all-content css-1ck9wyi"})
                t = ""
                for i in div.find_all("p"):
                    t += i.text + " "
                text.append(t)
    ...
    def thesun():
        try:
            ...
            all_divs = soup.find_all(
                "a", {"class": "teaser-anchor teaser-anchor--search"})
            for div in all_divs:
                links.append(div["href"])
            # Fetch all the necessary data
            # print(links)
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "article__timestamp"}).text)

                # Title of article

                title.append(
                    soup.find("h1", {"class": "article__headline"}).text)
    ...
    def koreannewsgazette():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("time", {"class": "entry-date published updated"}).text)
                # print(pub_date)
    ...
    def parool():
        try:
            ...
            all_divs = soup.find_all(
                "article", {"class": "fjs-teaser-compact teaser--compact"})
            for div in all_divs:
                links.append(domain_url+div.a["href"])
            # Fetch all the necessary data
            # print(links)
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    str(soup.find("time", {"class": "artstyle__production__datetime"})["datetime"]))

                # Title of article
                title.append(
                    soup.find("h1", {"class": "artstyle__header-title"}).text)
    ...
    def shabiba():
        ...
        # print(links)
            ...
            for link in links:
              ...
              # Published Date
                pub_date.append(
                    soup.find("span", {"class": "text-muted"}).text)
    ...
    def sabah():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "textInfo align-center"}).text)
                # print(pub_date)
    ...
    def swissinfo():
        try:
            ...
            for link in links:
                ...
                # Title of article
                title.append(
                    soup.find("h1", {"class": "si-detail__title"}).text)
                # Published Date
                pub_date.append(
                    soup.find("time", {"class": "si-detail__date"})["datetime"])
                # print(pub_date)
    ...
    def dziennik():
        try:
            ...
            for link in links:
                ...
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "datePublished"}).text)
                # print(pub_date)
    ...
    def hungary():
        try:
            ...
            try:
                ...
                return
            all_divs = soup.find_all(
                "h1", {"class": "kJ eR eF eT eU eV eW kI eZ"})
            for div in all_divs:
                links.append(div.a["href"])
            # Fetch all the necessary data
            # print(links)
            ...
            for link in links:
                ...
                # Title of article
                title.append(
                    soup.find("h1", {"class": "kH eR eF eT eU eV eW fa eZ kG"}).text)
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "eE f5 eT eU eV eW fa fh"}).text)
                # print(pub_date)

                # Text of article
                div = soup.find(
                    "div", {"class": "eA f- fG eu eT he hf eW hg lp"})
                t = ""
                for i in div.find_all("p"):
                    t += i.text + " "
                text.append(t)
    ...
    def jauns():
        try:
            ...
            for link in links:
                ...
                # Title of article
                title.append(
                    soup.find("span", {"class": "heading__text"}).text)
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "meta-info__publish-date-full"}).text)
                # print(pub_date)
    ...
    def pulse():
        try:
            ...
            for link in links:
                ...
                # Title of article
                title.append(
                    soup.find("h1", {"class": "article-headline "}).span.text)
                # print(title)
                # Published Date
                pub_date.append(soup.find(
                    "time", {"class": "detail-article-date date-type-publicationDate"})["datetime"])
                # print(pub_date)
    ...
    def jamaicaobserver():
        try:
            ...
            for link in links:
                ...
                # Title of article
                title.append(
                    soup.find("div", {"class": "headline col-12"}).text)
                # Published Date
                pub_date.append(
                    soup.find("div", {"class": "article-pubdate"}).text)
                # print(pub_date)
    ...
    def independent():
        try:
            ...
            try:
                ...
                return
            all_divs = soup.find_all(
                "a", {"class": "c-card1-textlink -d:b -as:1"})
            for div in all_divs:
                links.append(div["href"])
            # Fetch all the necessary data
            # print(links)
            ...
            for link in links:
                try:
                    ...
                    # Title of article
                    title.append(
                        soup.find("h1", {"class": "title1-main"}).text)
                    # Published Date
        ...
        try:
            ...
            for keyword in keywords:
                try:
                    ...
                    err = "Bllomberg quint: err: Failed to access main url: " + \
                        url + keyword + " and convert it to soup object"
                    err_logs.append(err)
                    return None
            ...
            for link in links:
                try:
                    ...
                    try:
                        ...
                        err = "Bloomberg Quint: err: Failed to access main url: " + \
                            link + " and convert it to soup object"
                        err_logs.append(err)
                    # return None
                    pub_date.append(soup.find("time", {
                                    "class": "desktop-only published-info-module__updated-on__2JWey"}).string)
                    title.append(soup.find(
                        "h1", {"class": "story-base-template-m__story-headline__2cNwS"}).text)
                    t = [i.text for i in soup.find_all(
    ...
    def ecns():
        try:
            ...
            try:
                ...
                err = "ecns: err: Failed to access search url: " + \
                    queryUrl + " and convert it to soup object"
                err_logs.append(err)
                exit()
            # with open("response.xml", "wb") as f:
    ...
    def energy_voice():
        try:
            ...
            for link in links:
                ...
                title.append(
                    soup.find("h1", {"class": "title entry-title"}).text)
                # print(title)
                # Published Date
                pub_date.append(
                    soup.find("span", {"class": "post-timestamp__published"}).text)
                # print(pub_date)
    ...
    def euroNews():
        try:
            ...
            try:
                ...
                err = "euroNews: err: Failed to access search url: " + \
                    queryUrl + " and convert it to soup object"
                err_logs.append(err)
            # with open("response.html", "wb") as f:
            #     f.write(pageSource.content)
            ...
            for link in links:
                ...
                #     f.write(pageSource.content)
                # break
                requiredTag = parsedSource.find(
                    "div", class_="c-article__full_article")
                if requiredTag:
    ...
    def theFreePressJournal():
        try:
            ...
            try:
                ...
                err = "theFreePressJournal: err: Failed to access search url: " + \
                    queryUrl + " and convert it to soup object"
                err_logs.append(err)
            # with open("response.html", "wb") as f:
            #     f.write(pageSource.content)
            ...
            theFreePressJournalDF = theFreePressJournalDF.drop_duplicates(subset=[
                                                                          "link"])
    ...
    def aylien():
        ...
        def fetch_news_stories(api_instance, params={}):
            ...
            while (stories is None or len(stories) > 0) and len(fetched_strories) < 1300:
                ...
                print("Fetched %d stories.Total story count so far : %d" %
                      (len(stories), len(fetched_strories)))
        ...
        for i in stories:
            ...
            pub_date.append(
                "-".join(str(i["published_at"]).strip().split(" ")[0].split("-")[::-1]))
            link.append(i["links"]["permalink"])
            scraped_date.append(today)

        df = pd.DataFrame({"text": text, "link": link, "publish_date": pub_date,
                          "scraped_date": scraped_date, "title": title})
        return df
    ...
    def dw():
        try:
            ...
            for link in links:
                ...
                try:
                    # title_ele = l_soup.find("h1", {"class": title_h1_class})
                    # There exists only 1 h1 ele in the page
                ...
                # Scraping the published date
                try:
                    date_ele_container = l_soup.find(
                        "ul", {"class": date_ul_class})
                    date_text = date_ele_container.text.split("\n")[2]
                    #date_text = (date_text.split('/'))[-1]
                    #date_text = date_text.replace(" Updated: ", "")
                    # The date_text could be further modified to represent a proper date format
                    data.append(date_text.replace(".", "-").strip())
                except:
                    err = scrapper_name + ": err: Failed to find date in page. Link: " + link
                    err_logs.append(err)
                    continue  # drops the complete data if there is an error
        ...
        try:
            ...
            for link in links:
                ...
                date_ele = l_soup.find(
                    "section", {"class": date_section_class})
                try:
                    date_text = date_ele.text
                    # date_text = (date_text.split('\n'))[-2]
                    #date_text = date_text.replace(" Updated: ", "")
                ...
                try:
                    para_ele = (l_soup.findAll(
                        "section", {"class": para_section_class}))[-1]
                    # para_ele = para_ele.strip("\n")
                    data.append(para_ele.text)  # Need to make this better
                except:
                    err = scrapper_name + ": err: Failed to find paragraph in page. Link: " + link
    ...
    def astanatimes():
        try:
            ...
            for link in links:
                ...
                try:
                    # title_ele = l_soup.find("div", {"class": title_div_class})
                    title_ele_container = l_soup.find(
    ...
    def bankokpost():
        try:
            ...
            for link in links:
                ...
                try:
                    title_ele = (l_soup.find(
                        "div", {"class": title_div_class})).find("h1")
                    text = title_ele.text
                    text = text.strip("\n")
                    text = text.strip("\t")
                    data.append(text)
    ...
    def globalnewswire():
        try:
            ...
            source = requests.get(
                "https://www.globenewswire.com/search/keyword/IPO")
            content = BeautifulSoup(source.content, 'html.parser')
            head = content.find(
                "div", style="margin-top: 2.3rem; min-height: 1000px")
            links = ["https://www.globenewswire.com"+str(i.a.get('href')) for i in head.find_all(
                'div', {"class": "pagging-list-item-text-container"})]
            list(set(links))
            ...
            for i in links:
                try:
                    ...
                    err_links.append(i)
                    continue
            df = pd.DataFrame({"text": data, "link": link, "publish_date": published,
                              "scraped_date": scraped, "title": title})
            df = FilterFunction(df)
    ...
    def headlinesoftoday():
        try:
            ...
            latest_links = [i.get('href') for i in content.find_all(
                "a", {"class": "post-url post-title"})]
            ...
            for i in latest_links:
                try:
                    ...
                    continue
            frame = pd.DataFrame(
                {"text": data, "link": link, "publish_date": published, "scraped_date": scraped, "title": title})
    ...
    def africabusinessinsider():
        try:
            ...
            date_time_class = ["detail-article-date",
    ...
    def africanmarkets():
        try:
            ...
            for link in links:
                ...
                try:
                    date_ele = l_soup.find(
                        "time", {"itemprop": date_time_itemprop})
                    date_text = date_ele.text
                    date_text = date_text.strip("\n")
                    date_text = date_text.strip()
    ...
    def dominicantoday():
        try:
            ...
            for link in links:
                ...
                try:
                    para_ele = (l_soup.findAll(
                        "span", {"itemprop": para_span_itemprop}))[-1]
                    para_text = para_ele.text
                    para_text = para_text.strip("\n")
                    para_text = para_text.strip()
                    data.append(para_text)
    ...
    def investmentu():
        ...
        try:
            ...
            # Class names of the elements to be scraped
            # Class name of div containing the a tag
    ...
    def altassets():
        ...
        try:
            ...
            # Class names of the elements to be scraped
            # Class name of div containing the a tag
            h3_class = "entry-title td-module-title"
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_h1_class = "entry-title"
    ...
    def indianexpress():
        try:
            ...
            for link in links:
                ...
                try:
                    date_ele = l_soup.find(
                        "span", {"itemprop": date_span_itemprop})
                    date_text = date_ele.text
                    date_text = date_text.strip("\n ")
                    date_text = " ".join(date_text.split()[:3])
                    #date_text = date_text.replace(" Updated: ", "")
    ...
    def hindustannewshub():
        try:
            ...
            date_time_class = ["entry-date updated td-module-date"]
            para_div_class = [
                "td_block_wrap tdb_single_content tdi_21 td-pb-border-top td_block_template_1 td-post-content tagdiv-type"]
    ...
    def ndtv():
        try:
            ...
            for link in links:
                ...
                try:
                    date_ele = l_soup.find(
                        "span", {"itemporp": date_span_itemprop})
                    date_text = date_ele.text
                    date_text = date_text.strip("\n ")
                    date_text = " ".join(date_text.split()[:3])
                    #date_text = date_text.replace(" Updated: ", "")
    ...
    def detroitnews():
        try:
            ...
            for link in links:
                ...
                try:
                    title_ele = l_soup.find(
                        "h1", {"elementtiming": title_h1_elementtiming})
                    title_text = title_ele.text
                    title_text = title_text.strip("\n ")
                    data.append(title_text)
                except:
    ...
    def businessstandard():
        try:
                    ...
                    for a1 in a_all:
                        if ((a1['href'] != None) and (a1.text != None)):
                            ...
                            if (soup1.find("div", {"class": "story-content"}) != None):
                                articlebody = soup1.find(
                                    "div", {"class": "story-content"})
                                if (articlebody.find("span", {"class": "p-content"}) != None):
                                    span_class = articlebody.find(
                            ...
                            if (nd != None):
                                ...
                                pubDate1 = soup1.find(
                                    'div', {"class": "publish-date"}).h4.i.text
            ...
            df = pd.DataFrame({"text": text, "link": links, "publish_date": pub_date,
                              "scraped_date": scraped_date, "title": title})
        ...
        try:
            ...
            if (all_divs != None):
                for div1 in all_divs:
                    if (div1 != None):
                        date1 = div1.find(
                            "div", {"class": "queryly_item_description"})
                        ...
                        if (date1 != None):
                            ...
                            if (len(utext_main) >= 12):
                                temp1_month_year = utext_main[0:3] + \
                                    " "+utext_main[8:12]
                                ...
                                currentMonthandYear = str(
                                    monthdict[currentMonth])+" "+str(currentYear)
                                # print("\n\n...........\n\n")
            ...
            for ul in all_ul:
                ...
                for i in listitem:
                    ...
                    if (date1 != None):
                        ...
                        currentMonth = datetime.now().month
                        currentYear = datetime.now().year
                        monthdict = {1: "Januari", 2: "Februari", 3: "Maret", 4: "April", 5: "Mei", 6: "Juni",
                        ...
                        if (temp1_month_year == currentMonthandYear):
                            print(
                                "Matched....the news belong to current month, proceed with new fetch details")
                            pub_date.append(utext_main)
            ...
            if (articles != None):
                for article in articles:
                    ...
                    if (len(publishdateold) == 15):
                        publishdateold = publishdateold[0:3] + \
                            " "+publishdateold[11:15]
                    elif (len(publishdateold) == 14):
                        publishdateold = publishdateold[0:3] + \
                            " "+publishdateold[10:14]
                    ...
                    if (publishdateold == currentmonthandyear):
                        ...
                        if (h2find != None):
                            ...
                            if (href != None):
                                ...
                                title = soup2.find(
                                    'h1', {"class": "entry-title cb-entry-title entry-title cb-title"})
                                titletext = ""
                                if (title != None):
                    ...
                    df = pd.DataFrame(
                        {"text": text, "link": links, "publish_date": pub_date, "scraped_date": scrape_date, "title": titles})
    ...
    def thenewsminute():
        try:
            ...
            # Class names of the elements to be scraped
            # Class name of div containing the a tag
            div_class = "col-8 col-md-9 text-wrap"
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
    ...
    def alphastreet():
        try:
            ...
            # Class names of the elements to be scraped
            # Class name of div containing the a tag
            div_class = "post_wrapper post_wrap_170616 post_1"
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
    ...
    def bing_search():
        try:
            ...
            df1 = pd.DataFrame(
                columns=['title', 'link', 'publish_date', 'scraped_date'])
            return df1
    def marketing_interactive():
        try:
            print("marketing-interactive")
            url = "https://www.marketing-interactive.com/component/search/?searchword=ipo&searchphrase=all&Itemid=406"
            domain_url = "https://www.marketing-interactive.com/"
            ...
            #h1_div_class = "col-xs-12"
            title_h1_class= "article-title"
            # date_li_class= ["entry-date"]
            para_section_class=["article-content"]
            links = []
            ...
            collection = []
            err_logs = []
            scrapper_name = "marketing-interactive"
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("marketing-interactive",df)
            return df
        except:
            print("marketing-interactive not working")
            not_working_functions.append("marketing-interactive")

    def gadgets360():
        try:
            print("gadgets360")
            url = "https://gadgets360.com/search?searchtext=ipo"
            domain_url = "https://gadgets360.com/"
            ...
            # Class names of the elements to be scraped
            div_class="caption_box"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_div_class= "lead_heading header_wrap"
            date_div_class= ["dateline"]
            para_div_class=["content_text row description"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("gadgets360",df)
            return df
        except:
            print("gadgets360 not working")
            not_working_functions.append("gadgets360")
    ...
    def interfax():
        try:
            print("interfax")
            url = "https://interfax.com/search/?q=ipo&how=d"
            domain_url = "https://interfax.com/"
            ...
            # Class names of the elements to be scraped
            div_class="item-search"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_h2_class= "h1"
            date_span_class= ["date"]
            para_div_class=["new-block"]
            links = []
            ...
            for link in links:
                ...
                try:
                    title_ele = l_soup.find("h2", {"class": title_h2_class})
                    title_text = title_ele.text
                    title_text = title_text.strip("\n ")
                    data.append(title_text)
                except:
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("interfax",df)
            return df
        except:
            print("interfax not working")
            not_working_functions.append("interfax")
    ...
    def businessgreen():
        try:
            print("businessgreen")
            url = "https://www.businessgreen.com/search?query=ipo&per_page=24&sort=relevance1&date=this_year"
            domain_url = "https://www.businessgreen.com/"
            ...
            title_h1_itemprop= "name"
            date_span_itemprop= ["datePublished"]
            para_div_class=["article-content-block"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("businessgreen",df)
            return df
        except:
            print("businessgreen not working")
            not_working_functions.append("businessgreen")

    def theglobeandmail():
        try:
            print("theglobeandmail")
            url = "https://www.theglobeandmail.com/search/?q=ipo&mode=all&S=relevant"
            domain_url = "https://www.theglobeandmail.com"
            ...
            # Class names of the elements to be scraped
            div_class="c-card"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_h1_class= "c-primary-title hl-3 hl-3-md font-pratt-bold c-primary-title-news"
            date_time_class= ["c-timestamp u-no-wrap acl-1 font-gmsans"]
            para_article_class=["l-article"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("theglobeandmail",df)
            return df
        except:
            print("theglobeandmail not working")
            not_working_functions.append("theglobeandmail")
    ...
    def beincrypto():
        try:
            print("beincrypto")
            url = "https://beincrypto.com/?s=ipo"
            domain_url = "https://beincrypto.com/"
            ...
            # Class names of the elements to be scraped
            div_class="flex text-xs mb-1 h-full"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_h1_class= "text-h4 my-3 normal-case lg:mb-5 lg:max-w-[800px] lg:font-bold lg:leading-[48px] lg:text-[44px]"
            
            para_div_class=["entry-content-inner"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("beincrypto",df)
            return df
        except:
            print("beincrypto not working")
            not_working_functions.append("beincrypto")

    def enterprisetalk():
        try:
            print("enterprisetalk")
            url = "https://enterprisetalk.com/?s=ipo"
            domain_url = "https://enterprisetalk.com/"
            ...
            # Class names of the elements to be scraped
            div_class="td_module_16 td_module_wrap td-animation-stack"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_h1_class= "entry-title"

            para_div_class=["td-post-content tagdiv-type"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("enterprisetalk",df)
            return df
        except:
            print("enterprisetalk not working")
            not_working_functions.append("enterprisetalk")
    ...
    def tribuneindia():
        try:
            print("tribuneindia")
            url = "https://www.google.com/search?domains=www.tribuneindia.com&sitesearch=www.tribuneindia.com&q=ipo"
            domain_url = "https://www.tribuneindia.com/"
            ...
            # Class names of the elements to be scraped
            div_class="MjjYud"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            
            
            para_div_class=["story-desc"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("tribuneindia",df)
            return df
        except:
            print("tribuneindia not working")
            not_working_functions.append("tribuneindia")

    def cityam():
        try:
            print("cityam")
            url = "https://www.cityam.com/?s=ipo"
            domain_url = "https://www.cityam.com/"
            ...
            # Class names of the elements to be scraped
            li_class="content-listing__content-item"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            date_time_class= ["date-time__time"]
            
            para_article_class=["content-container content-container__single"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("cityam",df)
            return df
        except:
            print("cityam not working")
            not_working_functions.append("cityam")
    ...
    def bollyinside():
        try:
            print("bollyinside")
            url = "https://www.bollyinside.com/?s=ipo"
            domain_url = "https://www.bollyinside.com/"
            ...
            # Class names of the elements to be scraped
            div_class="tdb_module_loop td_module_wrap td-animation-stack td-cpt-post"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            date_time_class= ["entry-date updated td-module-date"]
            
            para_div_class=["td_block_wrap tdb_single_content tdi_97 td-pb-border-top td_block_template_1 td-post-content tagdiv-type"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("bollyinside",df)
            return df
        except:
            print("bollyinside not working")
            not_working_functions.append("bollyinside")

    def eurekalert():
        try:
            print("eurekalert")
            url = "https://www.eurekalert.org/advancedSearch/335589"
            domain_url = "https://www.eurekalert.org/"
            ...
            # Class names of the elements to be scraped
            tr_class="post"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            ...
            for divtag in soup.find_all("tr", {"class": tr_class}):
                for a in divtag.find_all("a", href=True):
                    link = a["href"]  # Gets the link
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("eurekalert",df)
            return df
        except:
            print("eurekalert not working")
            not_working_functions.append("eurekalert")

    def wirtschaftsanzeiger():
        try:
            print("wirtschaftsanzeiger")
            url = "https://wirtschaftsanzeiger.com/search?q=ipo"
            domain_url = "https://wirtschaftsanzeiger.com/"
            ...
            # Class names of the elements to be scraped
            article_class="article-item"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            
            date_p_class= ["article-published-at"]
            para_div_class=["article-content"]
            links = []
            ...
            collection = []
            err_logs = []
            scrapper_name = "wirtschaftsanzeiger"
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("wirtschaftsanzeiger",df)
            return df
        except:
            print("wirtschaftsanzeiger not working")
            not_working_functions.append("wirtschaftsanzeiger")

    def valdostadailytimes():
        try:
            print("valdostadailytimes")
            url = "https://www.valdostadailytimes.com/search/?l=25&sd=desc&s=start_time&f=html&app=editorial&q=ipo&nsa=eedition"
            domain_url = "https://www.valdostadailytimes.com/"
            ...
            # Class names of the elements to be scraped
            div_class="card-container"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            title_h1_class= "headline"
            
            para_div_id=["article-body"]
            links = []
            ...
            collection = []
            err_logs = []
            scrapper_name = "valdostadailytimes"
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("valdostadailytimes",df)
            return df
        except:
            print("valdostadailytimes not working")
            not_working_functions.append("valdostadailytimes")


    def saltwire():
        try:
            print("saltwire")
            url = "https://www.saltwire.com/search/?search=ipo&order=newest"
    ...
    def taipeitimes():
        try:
            print("taipeitimes")
            url = "https://www.taipeitimes.com/News/list?section=all&keywords=ipo"
            domain_url = "https://www.taipeitimes.com/"
            ...
            # Class names of the elements to be scraped
            li_class="1"# Class name of div containing the a tag
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            
            para_div_class=["archives"]
            links = []

            for divtag in soup.find_all("li", {"data-page": li_class}):
                for a in divtag.find_all("a", href=True):
                    link = a["href"]  # Gets the link
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("taipeitimes",df)
            return df
        except:
            print("taipeitimes not working")
            not_working_functions.append("taipeitimes")
    ...
    def wealthx():
        try:
            print("wealthx")
            url = "https://www.wealthx.com/?s=ipo"
            domain_url = "https://www.wealthx.com/"
            ...
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            article_class="result"
            title_h1_class= "entry-title"
            date_span_class= ["meta-date date updated"]
            para_div_class=["content-inner"]
            ...
            print("wealthx not working")
            not_working_functions.append("wealthx")
    ...
    def macaubusiness():
        try:
            print("macaubusiness")
            url = "https://www.macaubusiness.com/?s=ipo"
    ...
    def technode():
        try:
            ...
            for divtag in soup.find_all("main",{"class":main_class}):
                for a in divtag.find_all("a", href=True):
                    link = a["href"]  # Gets the link
    ...
    def gadgetsndtv():
        try:
            print("gadgetsndtv")
            url = "https://www.gadgets360.com/search?searchtext=ipo"
            domain_url = "https://www.gadgets360.com/"
            ...
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            div_class="story_list search_listing row margin_b20"
            date_div_class= ["dateline"]
            para_div_class=["content_text row description"]
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("gadgetsndtv",df)
            return df
        except:
            print("gadgetsndtv not working")
            not_working_functions.append("gadgetsndtv")

    def asumetech():
        try:
            print("asumetech")
            url = "https://asumetech.com/?s=ipo"
            domain_url = "https://asumetech.com/"
            ...
            #h1_class = "_1Y-96"
            #h1_div_class = "col-xs-12"
            div_class="td_block_inner tdb-block-inner td-fix-index"
            date_time_class= ["entry-date updated td-module-date"]
            para_div_id=["body-text"]
            links = []
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("asumetech",df)
            return df
        except:
            print("asumetech not working")
            not_working_functions.append("asumetech")

    def vietnamnet():
        try:
            print("vietnamnet")
            url = "https://vietnamnet.vn/en/tim-kiem?q=ipo"
    ...
    df150 = marketing_interactive()
    df151 = gadgets360()
    df152 = reinsurancene()
    df153 = interfax()
    df154 = gulfbusiness()
    df155 = businessgreen()
    df156 = theglobeandmail()
    df157 = insideretail()
    df158 = seenews()
    df159 = globenewswire()
    df160 = autonews()
    df161 = shorttermrentalz()
    df162 = beincrypto()
    df163 = enterprisetalk()
    df164 = capacitymedia()
    df165 = tribuneindia()
    df166 = cityam()
    df167 = kenyanwallstreet()
    df168 = bollyinside()
    df169 = eurekalert()
    df170 = wirtschaftsanzeiger()
    df171 = valdostadailytimes()
    df172 = saltwire()
    df173 = taipeitimes()
    df174 = thesundaily()
    df175 = digitaljournal()
    df176 = wealthx()
    df177 = asiafinancial()
    df178 = macaubusiness()
    df179 = technode()
