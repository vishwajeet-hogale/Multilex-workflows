...
# koreatimes() script
    ...
    def zhitongcaijing(keyword, page):
        try:
            print("zhitongcaijing")
            Errors["zhitongcaijing"]=[]

            url = f"https://www.zhitongcaijing.com/?index={keyword}&page={page}"
            domain_url = "https://www.zhitongcaijing.com/"
            headers = {
                "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:78.0) Gecko/20100101 Firefox/78.0",
                "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
                'sec-fetch-site': 'none',
            ...
            try:
                ...
                print("zhitongcaijing not working")
                not_working_functions.append('zhitongcaijing')
                err = "Main link did not load: "+ url
                Errors["zhitongcaijing"].append(err)
                return
            
            try:
                all_divs = soup.find_all("div", {"class": "info-item-content-title"})
                for div in all_divs:
                    links.append(domain_url+div.a["href"])
            except:
                if len(links)==0:
                    print("zhitongcaijing not working")
                    not_working_functions.append('zhitongcaijing')
                    Errors["zhitongcaijing"].append("Extraction of link not working.")
                    return
            ...
            def getarticles(link):
                ...
                try:
                    title_ele =l_soup.find("h1", {"class": "news-title"})
                    title_text =title_ele.text
                    title_text=translatedeep(title_text)
                    data.append(title_text)
                except:
                    err["link"]=link
                    err['title']="Error"
                ...
                # Scraping the published date
                try:
                    date1 = l_soup.find("div", {"class": "news-authnor-left-box"})
                    for date_ele in date1.find_all("span"):
                        date_text = date_ele.text
                ...
                # Scraping the paragraph
                try:
                   para_ele = l_soup.find("div", {"class": "news-body-content"})
                   for para in para_ele.find_all("p"):
                       para_text = para.text
                   para_text=translatedeep(para_text)
                   data.append(para_text)
                except:
                    err["link"]=link
                    err['text']="Error"
            ...
            # print(err_logs) # For debugging - to check if any errors occoured
            df = FilterFunction(df)
            emptydataframe("zhitongcaijing", df)
            # df  = link_correction(df)
